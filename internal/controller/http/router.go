...
func NewRouter(app *fiber.App, l logger.Interface, t usecase.Translation) {
	...
	// Prometheus предлагает нативную поддержку для сбора метрик от приложений,
	// что делает его идеальным выбором для мониторинга Go-приложений.
	//
	// Эндпойнт GET "/metrics" работает.
	// Метрики выводит после любого запроса (кроме самого себя)
	// Prometheus metrics
